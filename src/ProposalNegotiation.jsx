...
const renderText = (text) => {
  ...
  return text.split("\n").map((line, i) => {
    ...
    if (headerMatch) {
      ...
      return <p key={i} className="neg-bubble-header"><strong>{headerMatch[1]}</strong>{rest ? ` ${rest}` : ""}</p>;
    }
    const parts = line.split(/\*\*(.+?)\*\*/g);
    return <p key={i} className="neg-bubble-line">{parts.map((part, j) => j % 2 === 1 ? <strong key={j}>{part}</strong> : part)}</p>;
  });
};
      ...
      <div className={`neg-bubble ${isMine ? "neg-bubble--freelancer" : "neg-bubble--client"} ${msg.type === "counter" ? "neg-bubble--counter" : ""}`}>
        ...
        {(msg.bid || msg.counterBid) && (
          <div className="neg-terms-row">
            <div className="neg-term"><span className="neg-term-label">Bid</span><span className="neg-term-val neg-term-val--green">${msg.bid || msg.counterBid}</span></div>
            <div className="neg-term-divider" />
            <div className="neg-term"><span className="neg-term-label">Timeline</span><span className="neg-term-val">{msg.timeline || msg.counterTimeline} days</span></div>
          </div>
        )}
      </div>
    </div>
...
const SignatureCanvas = ({ onSave, onClear, label }) => {
  ...
  return (
    <div className="neg-sig-wrap">
      {label && <div className="neg-sig-label">{label}</div>}
      <canvas ref={canvasRef} width={260} height={90} className="neg-sig-canvas"
        onMouseDown={start} onMouseMove={draw} onMouseUp={end} onMouseLeave={end}
        onTouchStart={start} onTouchMove={draw} onTouchEnd={end}
      />
      <div style={{ display: "flex", gap: "8px", marginTop: "4px", alignItems: "center" }}>
        <button className="neg-sig-clear" onClick={clear}>Clear</button>
        {hasSig && <span className="neg-sig-done">✓ Drawn</span>}
      </div>
    </div>
  );
};
    ...
    {sigData ? (
      <div>
        ...
        <div style={{ display: "flex", gap: "8px", marginTop: "6px", alignItems: "center" }}>
          <span className="neg-sig-done">✓ Signed</span>
          {canSign && <button className="neg-sig-clear" onClick={onClear}>Clear</button>}
        </div>
      </div>
    ) : canSign ? (
...
const ProposalSidebar = ({ project, currentTerms, negotiationStatus, onAccept, onReject, isFinalized, deal, isClient }) => (
  <div className="neg-sidebar">
    <div className="neg-sidebar-card">
      <div className="neg-sidebar-section-title">Project Info</div>
      <div className="neg-sidebar-info-row"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/></svg><span>{project.title}</span></div>
      <div className="neg-sidebar-info-row"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg><span>{project.client_name}</span></div>
      <div className="neg-sidebar-info-row"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#1a56db" strokeWidth="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg><span className="blue">{project.budget}</span></div>
    </div>
    <div className="neg-sidebar-card">
      <div className="neg-sidebar-section-title">{isFinalized ? "Agreed Terms" : "Latest Terms"}{isFinalized && <span className="neg-finalized-chip">✓ Final</span>}</div>
      <div className="neg-terms-grid">
        <div className="neg-terms-item"><span className="neg-terms-item-label">Bid Amount</span><span className={`neg-terms-item-val ${isFinalized ? "green" : ""}`}>${isFinalized ? deal.bid : currentTerms.bid}</span></div>
        <div className="neg-terms-item"><span className="neg-terms-item-label">Timeline</span><span className="neg-terms-item-val">{isFinalized ? deal.timeline : currentTerms.timeline} days</span></div>
        <div className="neg-terms-item"><span className="neg-terms-item-label">Budget</span><span className="neg-terms-item-val gray">{project.budget}</span></div>
        <div className="neg-terms-item"><span className="neg-terms-item-label">Deadline</span><span className="neg-terms-item-val gray">{project.deadline}</span></div>
      </div>
    </div>
    {!isFinalized && isClient && negotiationStatus === "awaiting_client" && (
      <div className="neg-sidebar-card neg-sidebar-actions">
        ...
        <button className="neg-accept-btn" onClick={onAccept}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>Accept Terms</button>
        <button className="neg-reject-btn" onClick={onReject}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>Reject Proposal</button>
      </div>
    )}
    {!isFinalized && !isClient && negotiationStatus === "awaiting_freelancer" && (
      <div className="neg-sidebar-card neg-sidebar-actions">
        ...
        <button className="neg-accept-btn" onClick={onAccept}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>Accept These Terms</button>
        <button className="neg-reject-btn" onClick={onReject}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>Reject Proposal</button>
      </div>
    )}
    <div className="neg-sidebar-card">
      ...
      <div className="neg-timeline">
        <div className="neg-timeline-item neg-timeline-item--done"><div className="neg-tl-dot" /><div><p className="neg-tl-label">Proposal Sent</p><p className="neg-tl-time">Done</p></div></div>
        <div className={`neg-timeline-item ${negotiationStatus !== "pending" ? "neg-timeline-item--done" : "neg-timeline-item--active"}`}><div className="neg-tl-dot" /><div><p className="neg-tl-label">Negotiating</p><p className="neg-tl-time">{negotiationStatus !== "pending" ? "Ongoing" : "Waiting…"}</p></div></div>
        <div className={`neg-timeline-item ${isFinalized ? "neg-timeline-item--done" : "neg-timeline-item--inactive"}`}><div className="neg-tl-dot" /><div><p className="neg-tl-label">Deal Finalized</p><p className="neg-tl-time">{isFinalized ? "Done" : "Pending"}</p></div></div>
        <div className={`neg-timeline-item ${isFinalized ? "neg-timeline-item--active" : "neg-timeline-item--inactive"}`}><div className="neg-tl-dot" /><div><p className="neg-tl-label">Project Active</p><p className="neg-tl-time">{isFinalized ? "Active" : "Pending"}</p></div></div>
      </div>
    </div>
  </div>
);
...
const CounterForm = ({ initialBid, initialDays, onSubmit, onCancel, isLoading }) => {
  ...
  return (
    <div className="neg-counter-form">
      <div className="neg-counter-form-header"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2v4m0 12v4M4.93 4.93l2.83 2.83m8.48 8.48 2.83 2.83M2 12h4m12 0h4M4.93 19.07l2.83-2.83m8.48-8.48 2.83-2.83"/></svg>Send Counter Offer</div>
      <div className="neg-counter-fields">
        <div className="neg-counter-field"><label>Bid Amount</label><div className="neg-cf-input-wrap"><span>$</span><input type="number" value={bid} onChange={e => setBid(e.target.value)} min="1" /></div></div>
        <div className="neg-counter-field"><label>Delivery Days</label><div className="neg-cf-input-wrap"><input type="number" value={days} onChange={e => setDays(e.target.value)} min="1" /><span>days</span></div></div>
      </div>
      ...
      <div className="neg-counter-actions">
        ...
        <button className="neg-submit-counter-btn" onClick={() => onSubmit({ bid: parseInt(bid), timeline: parseInt(days), note })} disabled={isLoading}>
          {isLoading ? <span className="neg-spinner" /> : <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>Send Counter</>}
        </button>
      </div>
    </div>
  );
...
async function generateAIDraft(context) {
  ...
  const historyText = context.messages.filter(m => m.type !== "system").map(m => {
  ...
  const systemPrompt = context.isClient
    ? `You are ${context.clientName}, a client reviewing a freelancer's proposal for "${context.projectTitle}". Your budget is ${context.budget}. Write a short, professional, realistic reply as the CLIENT. Keep it 2-3 sentences.`
    : `You are a freelancer negotiating professionally with client ${context.clientName} for project "${context.projectTitle}". Write a concise, friendly reply. Keep it 2-3 paragraphs.`;

  const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
    method: "POST",
    headers: { "Content-Type": "application/json", "Authorization": "Bearer " + GROQ_API_KEY },
    body: JSON.stringify({ model: "llama-3.1-8b-instant", max_tokens: 350, temperature: 0.7, messages: [{ role: "system", content: systemPrompt }, { role: "user", content: `Conversation so far:\n\n${historyText}\n\nNow write your reply:` }] }),
  });
...
export default function ProposalNegotiation() {
  ...
  const proposalText = savedProposal.proposalText || "Hi, I'd love to work on your project.";
  ...
  const [signatures, setSignatures] = useState({ freelancer: null, client: null });
  ...
  useEffect(() => {
    if (!showContract || !proposal_id || !isFinalized) return;
    fetch(`http://localhost:3001/api/signatures/negotiation/${proposal_id}`, {
      headers: { Authorization: `Bearer ${token}` }
    })
      .then(res => res.json())
      .then(data => {
  ...
  }, [showContract, proposal_id, isFinalized]);
  ...
  // ── Detect role ──
  useEffect(() => {
    if (project && user) setIsClient(project.client_id === user.id);
  }, [project, user]);
  ...
  const saveNegotiation = async (updatedMessages, status = "ongoing", finalBid = null, finalTimeline = null) => {
    ...
    } catch (err) { console.error("Gagal save negotiation:", err); }
  };
  ...
  const negotiationStatus = isFinalized
    ? "finalized"
    : lastMsg?.sender === "client" ? "awaiting_freelancer" : "awaiting_client";
  ...
  const handleAISuggest = async () => {
    ...
    try {
      const text = await generateAIDraft({ isClient, clientName: project.client_name, freelancerName, projectTitle: project.title, budget: project.budget, messages });
      setComposerText(text);
    } catch {
      setComposerText("Thank you for your message. I'd like to discuss the terms further.");
    } finally { setIsAISuggesting(false); }
  };

  // ── Download PDF — pakai signatures dari DB ──
  const handleDownloadPDF = () => {
    ...
    const sigFreelancerHTML = signatures.freelancer
      ...
      : '<div style="height:60px;border-bottom:1px solid #cbd5e1;margin-top:8px;"></div>';
          ...
          <div className="neg-contract-modal">
            ...
            <div className="neg-contract-body">
              ...
              <div className="neg-contract-parties">
                <div className="neg-contract-party"><span className="neg-cp-label">Client</span><span className="neg-cp-val">{project.client_name}</span></div>
                <div className="neg-contract-arrow">↔</div>
                <div className="neg-contract-party"><span className="neg-cp-label">Freelancer</span><span className="neg-cp-val">{freelancerName}</span></div>
              </div>
              ...
              {/* ── DIGITAL SIGNATURES — TERPISAH PER USER ── */}
              <div className="neg-sig-section">
                <div className="neg-sig-section-title">Digital Signatures</div>
                <p className="neg-sig-hint">
                  Each party signs their own section individually on their own device.
                ...
                <div className="neg-sig-row">
                  <SigBox
                    ...
                    onSign={async (dataUrl) => {
                      try {
                        await fetch("http://localhost:3001/api/signatures", {
                          method: "POST",
                          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
                          body: JSON.stringify({ contract_type: "negotiation", contract_id: proposal_id, role: "freelancer", signature_data: dataUrl })
                        });
                    ...
                    onClear={async () => {
                      try {
                        await fetch(`http://localhost:3001/api/signatures/negotiation/${proposal_id}`, {
                    ...
                    onSign={async (dataUrl) => {
                      try {
                        await fetch("http://localhost:3001/api/signatures", {
                          method: "POST",
                          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
                          body: JSON.stringify({ contract_type: "negotiation", contract_id: proposal_id, role: "client", signature_data: dataUrl })
                        });
                ...
                {signatures.freelancer && signatures.client && (
                  <p className="neg-sig-both-done">✓ Both parties have signed. You can now download the contract.</p>
                )}
                {(!signatures.freelancer || !signatures.client) && (
                  <p style={{ fontSize: "12px", color: "#94a3b8", marginTop: "8px" }}>
                    {isClient
                      ? "Sign your section above. The freelancer will sign on their device when they open this contract."
                      : "Sign your section above. The client will sign on their device when they open this contract."}
                  </p>
                )}
              </div>
              ...
              <div className="neg-contract-actions">
                <button
                  ...
                  disabled={!signatures.freelancer || !signatures.client}
                  title={!signatures.freelancer || !signatures.client ? "Both parties must sign first" : "Download PDF"}
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                  Download PDF {(!signatures.freelancer || !signatures.client) && "(Sign first)"}
                </button>
    ...
    <div className="pg-page">
      ...
      <div className="neg-body">
        ...
        <div className="neg-layout">
          <div className="neg-chat-panel">
            ...
            <div className="neg-messages">
              {messages.map(msg => {
                    ...
                    <div key={msg.id} className="neg-system-msg">
                      <div className={`neg-system-msg-inner ${msg.systemLabel?.includes("Rejected") ? "neg-system-msg-inner--reject" : "neg-system-msg-inner--accept"}`}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                          {msg.systemLabel?.includes("Rejected") ? <><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></> : <polyline points="20 6 9 17 4 12"/>}
                        </svg>
                ...
                return (
                  <ProposalBubble key={msg.id} msg={msg} isMine={isMine} myInitials={myInitials} otherInitials={otherInitials}
            ...
            {!isFinalized && (
              <div className="neg-composer">
                {composerMode === "counter" ? (
                  <CounterForm initialBid={currentTerms.bid} initialDays={currentTerms.timeline} onSubmit={handleSendCounter} onCancel={() => setComposerMode("message")} isLoading={isSending} />
                ) : (
                  <>
                    <div className="neg-composer-toolbar">
                      ...
                      <button className="neg-toolbar-btn neg-toolbar-btn--counter" onClick={() => setComposerMode("counter")}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 2v4m0 12v4M4.93 4.93l2.83 2.83m8.48 8.48 2.83 2.83M2 12h4m12 0h4M4.93 19.07l2.83-2.83m8.48-8.48 2.83-2.83"/></svg>Counter Offer
                      </button>
                      <button className="neg-toolbar-btn neg-toolbar-btn--ai" onClick={handleAISuggest} disabled={isAISuggesting}>
                        {isAISuggesting ? <span className="neg-spinner neg-spinner--sm neg-spinner--dark" /> : <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg>}
                        AI Draft Reply
                      </button>
                    </div>
                    <div className="neg-composer-input-row">
                      <textarea className="neg-composer-textarea" placeholder={isClient ? `Reply to ${freelancerName}…` : `Reply to ${project.client_name}…`} value={composerText} onChange={e => setComposerText(e.target.value)} rows={3} onKeyDown={e => { if (e.key === "Enter" && (e.metaKey || e.ctrlKey)) handleSendMessage(); }} />
          ...
          <ProposalSidebar project={project} currentTerms={currentTerms} negotiationStatus={negotiationStatus} onAccept={handleAccept} onReject={handleReject} isFinalized={isFinalized} deal={deal} isClient={isClient} />
        </div>
      </div>
    </div>
  );
