    ...
    <nav className="pnav">
      <div className="pnav-inner">
        ...
        <div className="pnav-actions">
          {user ? (
            <button className="btn-pnav-login" onClick={() => navigate("/dashboard")}>
              Dashboard
            </button>
          ) : (
            <>
              <button className="btn-pnav-login" onClick={() => navigate("/login")}>
                Log In
              </button>
              <button className="btn-pnav-signup" onClick={() => navigate("/signup")}>
                Sign Up Free
              </button>
            </>
          )}
...
export default function ProjectPage() {
  ...
  const filtered = projects
    .filter((p) => {
      ...
      const matchSub =
        selectedSubs.length === 0 || selectedSubs.includes(p.sub_category);
      return matchSearch && matchSub;
    })
    .sort((a, b) => {
      if (sort === "Highest Budget")
        return budgetToNum(b.budget) - budgetToNum(a.budget);
      if (sort === "Lowest Budget")
        return budgetToNum(a.budget) - budgetToNum(b.budget);
      return 0;
    });
